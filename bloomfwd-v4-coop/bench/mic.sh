...
ALGS_SERIAL=("bloomfwd_opt_mic")
ALGS_PARALLEL=("bloomfwd_opt_mic_par_intrin")
THREADS=(30 60 90 120 150 180 210 244)
#THREADS=(244)
SCHED_CHUNKSIZE="dynamic,1"
OUTPUT_FILE=bench/res-opt-exectime/mic/lookup.csv # Benchmark output file.

cd $PHI_BLOOMFWD_DIR
mkdir -p bench/res-opt-exectime/mic/

# Clean old data files...
data_files=$(ls bench/res-opt-exectime/mic)
if [ ${#data_files} -gt 0 ]; then
	rm -f bench/res-opt-exectime/mic/*
fi
